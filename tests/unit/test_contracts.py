...
def test_a_hard_violation_makes_a_strategy_infeasible() -> None:
    ...
    hard = soft.model_copy(
        update={"constraint_violations": [_violation(ConstraintSeverity.HARD)]}
    )
...
def test_stress_headroom_must_tie_to_the_floor() -> None:
    ...
    result = StressResult(
        passed=False, headroom=Money.from_major("-2000000", USD), **kwargs  # type: ignore[arg-type]
    )
    assert not result.passed

    with pytest.raises(ValidationError):
        StressResult(
            passed=True, headroom=Money.from_major("-2000000", USD), **kwargs  # type: ignore[arg-type]
        )


def test_a_rejection_must_be_evidenced() -> None:
    action = ProposedAction(
        kind=ActionKind.AP_DEFER, rationale="stretch a supplier", delay_days=21
    )
    with pytest.raises(ValidationError):
