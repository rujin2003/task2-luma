...
def test_the_engine_toolset_is_honest_about_not_existing_yet() -> None:
    with pytest.raises(NotImplementedError):
        EngineToolset().get_covenant_status()
...
async def test_deferrable_total_excludes_protected_rows(toolset: FixtureToolset) -> None:
    ...
    assert payload.total_deferrable.minor_units == sum(
        row.amount.minor_units for row in deferrable
    )
