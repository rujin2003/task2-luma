...
class EngineToolset:
    """Person 1's implementation lands here, behind the signatures above.

    Every method raises until then. Person 2 builds against `FixtureToolset` and swaps at
    Merge Point 1 without touching a caller.
    """

    def __getattr__(self, name: str) -> object:
        if hasattr(Toolset, name):
            raise NotImplementedError(
                f"{name}() is not implemented yet -- the engine lands at Merge Point 1"
            )
