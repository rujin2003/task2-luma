...
class SupplierRiskAgent(Specialist):
    ...
    async def gather(self, tools: ScopedToolset) -> list[str]:
        candidates = await fetch(tools, "rank_deferral_candidates", DeferralCandidates, top_n=10)
        ...
        for supplier in self._suppliers():
            ...
            lines.append(cite(line, profile.references[0]) if profile.references else line)
        return lines
